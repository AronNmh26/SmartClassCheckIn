...
class _FinishScreenState extends State<FinishScreen> {
  ...
  Future<void> _submit() async {
    final learnedToday = _learnedController.text.trim();
    ...
    if (learnedToday.isEmpty) {
      _showError('Please share what you learned today.');
      return;
    }
    ...
    try {
      await _storageService.saveCheckOut(checkOut);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Class completion saved.')),
      );
      Navigator.of(context).pop();
    } catch (e) {
      if (!mounted) return;
      _showError('Failed to save completion.');
    } finally {
      if (mounted) {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          _SectionCard(
            title: 'Step 2: Post-class reflection',
            ...
            child: Column(
              children: [
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    labelText: 'Feedback for class or instructor (optional)',
                    border: OutlineInputBorder(),
          ...
          ElevatedButton.icon(
            ...
            label: Text(_isSaving ? 'Saving...' : 'Submit Completion'),
          ),
        ],
      ),
    );
